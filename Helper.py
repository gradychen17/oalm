...
def listOfDict_format_as_table(data,
                    keys,
                    header=None,
                    sort_by_key=None,
...
def printDict(dic, keys):
	'''
	Print dictionary with keys right aligned and values left aligned
	'''
	keywidth = max( len(key) for key in keys )		
	for key in keys:
		print '%*s: %-s' % (keywidth, key.capitalize(), dic[key])
...
if __name__ == '__main__':
    ...
    print listOfDict_format_as_table(data,
                          keys,
                          header,
                          sort_by_key,
                          sort_order_reverse)